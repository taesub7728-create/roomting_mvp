...
- var(--coral-action)은 흰 텍스트 또는 작은 흰 UI 콘텐츠의 대비 확보가
  필요한 액션 표면(버튼, 뱃지, 칩 등)에만 쓴다.
  (--coral은 흰 글씨 대비 2.73으로 WCAG AA 미달)
- 앱 아이콘, Splash 배경, 브랜드 일러스트, 독립 심볼처럼 ROOMTING을
  대표하는 브랜드 표면은 흰색 콘텐츠가 올라가더라도 var(--coral)을 쓴다.
  단순히 흰색 요소가 올라간다는 이유만으로 --coral-action을 적용하지 않는다.
