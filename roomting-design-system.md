...
#### 2026-07-31 톤 미세조정 + Accent 신설 — 근거

기존 논거(카테고리 내 공백, 타겟 정서 합치, 이미 각인 시작됨)는
그대로 유효하며 **핑크/코랄이라는 카테고리 자체를 바꾼 것이 아니다.**
아래 두 가지 실측 문제를 보완하기 위해 같은 계열 안에서 톤만
조정했다.

1. **WCAG 대비 미달**: 기존 `#F05A7E` 배경 위 흰색 텍스트 대비가
   2.73~3.25:1 수준으로, 헤드라인 같은 큰 굵은 텍스트조차 WCAG AA
   큰 텍스트 기준(3:1)을 안정적으로 통과하지 못함. 채도는 유지하고
   명도만 65%→약 63%로 낮춰 흰 텍스트 대비를 3.1:1 이상으로 확보
   (`#F26559`). Hue 변화는 1.7°에 불과해 육안상 같은 코랄로 인식됨
   (redmean 60.3, 참고: Airbnb 자체 리브랜딩 이동폭 redmean 68.1).
2. **Airbnb Rausch(#FF5A5F)와의 근접**: 기존 팔레트의 Accent 후보
   (#F35B4F)가 Airbnb 클래식 Rausch와 hue 6°, redmean 30.8로
   과도하게 근접해 있었음. Airbnb·당근마켓(#FF6600, 주황)·직방
   (#FFD46C, 옐로우) 세 브랜드 모두와 최대한 이격되는 hue(약 11°)로
   재산정해 `#ED6445`로 확정 (Airbnb redmean 52로 개선, 당근마켓과는
   redmean 104로 원래도 충분히 안전).

이번 조정은 브랜드 정체성(카테고리·타겟 정서·기축적된 자산) 논거를
대체한 것이 아니라 위 두 실측 문제에 대한 보정이므로, "위 근거를
대체할 만한 새로운 논거가 아니면 반영하지 않는다"는 원칙에 위배되지
...
- **[완료 2026-07-31]** 브랜드 컬러 톤 조정 및 Accent 신설 (2번 섹션 "2026-07-31 톤 미세조정 + Accent 신설" 근거 참조).
  ...
  `var(--pink)`을 참조하는 37개 파일(버튼·선택 상태·CTA 등)에 자동 반영됨.
  `--ink`/`--paper`는 변경하지 않음.
  **미해결**: 스플래시 컴포넌트(`Splash.jsx`/`Splash.css`, 7번 섹션)를 확인한 결과
  배경은 `var(--paper)`, 워드마크 텍스트는 `var(--ink)`를 쓰고 있어 이번 `--pink`
  교체가 스플래시 화면 자체의 시각적 변화로는 이어지지 않음. 스플래시에서 유일하게
  브랜드 컬러가 쓰이는 지점은 로고 마크 이미지(`src/assets/roomting-symbol.svg`,
  fill/stroke `#F05A7E` 하드코딩)인데, 이번 작업 범위에는 SVG 자산 재작업이
  포함되지 않아 **로고 SVG는 구 색상(`#F05A7E`)으로 남아있음** — 관련 SVG
  8개 파일(`roomting-symbol*.svg`, `roomting-icon-*.svg`, `roomting-lockup.svg`,
  `public/favicon.svg`) 일괄 갱신은 별도 작업으로 진행 필요.
