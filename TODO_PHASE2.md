...
# 진행 상태 (2026-08-07 마감 기준) — ⚠ 이 섹션은 그 시점 기록이다

> **⚠ 2026-08-11 갱신: 여기부터 읽지 말 것.** 이 섹션의 "다음에 할 일"은 022~030 적용
> 이전 기준이라 현재와 다르다. **현재 우선순위는 문서 맨 끝
> 「★★ 우선순위 변경 (2026-08-11) — 영업 개시 최소 조건」에 있다.**
> 이 섹션은 그때의 판단 근거를 남기기 위해 그대로 보존한다.
...
| **P2** | 실제 백업 완료 | ✅ 2026-08-11 |
| **P3** | manifest 작성 (행 수 대조 가능) | ✅ 2026-08-11 |
| **P4** | 복원 절차 검증 (`begin`/`rollback` 리허설) | ✅ **PASS** 2026-08-11 |
| **P5** | 중복 0건 | ✅ Q6 = 0 |
| **P6** | 031 원문과 DB 상태 일치 | ✅ Q8 = 5/5 |
| **P7** | rollback 경계 문서화 (lossless 경계) | ✅ 위 4번 항목 |
| **P8** | 031 ↔ 034 권한 중복 정리 | ✅ 2026-08-11 (아래) |

# ★ 031 READY (2026-08-11) — P1~P8 전부 PASS

**백업·복원 관점에서 031 은 READY 다.**

★ **다만 READY ≠ 적용 준비 완료.** 031:212-217 이 "이 migration 과 같은 배포에 포함"이라고
못박은 **프론트 작업이 남아 있다**(아래 「031 프론트 작업」). 그리고 2026-08-11 우선순위
변경으로 **031 자체가 뒤로 밀렸다**(문서 맨 끝 「영업 개시 최소 조건」 참고).

## 백업 (P2)

| 항목 | 값 |
| --- | --- |
| 위치 | `C:\Users\user\Desktop\roomting_backups\2026-08-11_pre031\` (**저장소 밖**) |
| 파일 | `backup.json` / `manifest.md` |
| `captured_at` | `2026-08-11T05:44:55.403515+00:00` |
| 대상 | restore target 5 + `requests` snapshot-only |

★ **백업 파일의 내용은 이 저장소 어디에도 기록하지 않는다.** 저장소가 public 이고
`chat_messages` 에 실제 대화 내용이, 여러 테이블에 고객 UUID 가 들어 있다.
`.gitignore` 로 방어하지 않고 **디렉터리 자체를 저장소 밖에 뒀다.**

`backup.json` 의 `meta.schema_state` 는 `unique_constraint_present=true` /
`trigger_fn_is_increment_plus_one=true` / `insert_policy_has_5_limit=false` —
즉 **031 미적용 상태에서 뜬 백업**이며, 트리거가 증분(`+1`) 형태라 복원 시 카운터 원복이
필요하다는 사실이 백업 파일 자체에 기록돼 있다.

## P4 리허설 결과 — PASS

| 항목 | 결과 |
| --- | --- |
| drift gate | **PASS** — 6/6 canonical equality 동일 |
| V1 rows | **PASS** — properties 2/2, images 4/4, favorites 1/1, rooms 2/2, messages 3/3 |
| V2 counter (⑥ 이후) | **PASS** — mismatch 0 |
| V2b step6 fix | **PASS** — 1 (증분 트리거가 실제로 발동했다는 증거) |
| V3 orphans | **PASS** — 4종 전부 0 |
| **V4 canonical equality** | **PASS** — 5개 테이블 전부 true |
| 롤백 | 실행 완료. 트랜잭션 abort 후 정리 |

★ **V4 가 이번 리허설의 핵심 결과다.** canonical JSON equality 가 참이면 행 수·UUID 집합·
정렬·`created_at`·nullable timestamp·본문·boolean·`image_url` 문자열이 **전부 백업과 동일**
하게 복원됐다는 뜻이다. V1/V2/V3 만으로는 "행 수와 FK 는 맞는데 값이 틀린" 경우를 잡지 못한다.

### 스크립트 결함 1건 — 측정에는 영향 없음, 재실행 불필요

최초 실행본의 `verdict` 계산이 `format('%s', boolean)` 결과를 `'true'` 문자열과 비교해
측정값이 전부 정상인데도 `★ FAIL` 을 출력했다.

★ **Postgres 에서 boolean 의 출력 함수는 `t`/`f` 를 내고, `'true'`/`'false'` 는 `::text`
캐스트의 결과다.** `format('%s')` 는 출력 함수를 쓰므로 `t` 가 나온다.
(`||` 연결은 캐스트를 써서 `true` 가 나온다 — 이 비대칭이 함정이다.)

**재실행하지 않았다.** `verdict` 는 이미 계산된 측정값을 문자열로 다시 비교한 **파생 표시값**
이라 측정 경로에 개입하지 않는다. 다섯 측정값이 전부 독립적으로 출력됐고 전부 기대와
일치했으며, `false` 가 `t` 로 렌더될 경로는 존재하지 않는다. 앞선 ①②⑥ 이 예외 없이
실행됐다는 사실도 추가 증거다.

수정본은 **측정 / 판정 / 표시 3계층으로 분리**했다 — 판정은 변수끼리 직접 비교하고
문자열을 거치지 않는다. 기대 행 수를 정규식 문자열에 박아두던 V1 검사도 같은 성격의
결함이라 정수 비교로 함께 바꿨다.

→ 스크립트 보존 위치: **`supabase/restore_rehearsal_pre031.sql`**
   (migration 이 아니라 도구. **백업 데이터는 포함하지 않는다** — 실행 시점에만 붙여넣는다)

## ★ 백필(031:182-187)은 현재 no-op 다 — Q7 실측 근거

031:179 가 "데이터 수정이라 CLAUDE.md 11번에 따라 별도 승인"이라고 한 백필인데,
**preflight Q7 실측으로 바꿀 행이 0건임이 이미 확인된다.**

Q7 결과가 `cc193972` 만 `counter 1 / rows 1 / distinct 1` 이고 나머지 9건이 전부 0 이었다.
즉 **10행 전부 `count(*) = count(distinct realtor_id)`** 이므로 백필 전후 값이 같다.

→ 지금 적용하면 백필은 승인 절차만 남고 **실제 변경은 0건**이다. 031:189-196 의
"백필 전 확인" 쿼리를 적용 직후 한 번 돌려 0행임을 재확인하고 넘어가면 된다.
(중개사가 같은 요청에 매물을 2개 이상 보내기 시작하면 그때부터 두 값이 갈린다.)

## 031 프론트 작업 (미착수)

031:212-217 이 **"이 migration 과 같은 배포에 포함"** 이라고 명시한 항목이다.
백업만 보고 적용하면 화면이 깨진다.

| # | 파일 | 변경 |
| --- | --- | --- |
| 1 | `properties.api.js:28` | `23505` 문구 제거 → `42501` 을 "최대 5개까지 제안할 수 있어요"로 안내 |
| 2 | `RealtorDashboard.jsx:132` | `if (r.my_response_count > 0) return false` **한 줄 삭제** → "내가 N개 제안함" 배지 |
| 3 | `RealtorRespond.jsx` | 제출 후 폼 초기화 → 최대 5개까지 이어서 추가 |
| 4 | `ResponseStatus.jsx` | 부동산별 그룹핑(아코디언). 키는 `realtor_id`(동명 사무소 혼입 방지) |
| 5 | `RealtorDashboard` | "응답 N건" → **"N곳 응답"** (`response_count` 의미 변경 반영) |
...
# ★★ 우선순위 변경 (2026-08-11) — 영업 개시 최소 조건 ★★

> **다음 세션은 이 섹션부터 읽는다.** 위쪽의 「진행 상태 (2026-08-07 마감 기준)」은
> 그 시점 기록이고 현재 우선순위가 아니다.

**시점 고정(11월 영업)을 버리고 최대한 빠른 완성으로 방향을 바꿨다.**

## 다음 작업 순서 — 이 4개가 영업 개시 최소 조건이다

| # | 항목 | 상태 |
| --- | --- | --- |
| **1** | **중개사 가입 버그** | ✅ **조사 완료 (2026-08-11) — latent 판정.** 아래 「Auth 운영 계약」 |
| **2** | **신규 중개사 승인 플로우 검증** — 새 테스트 계정으로 signup → application → admin 승인 → realtor 접근 전 흐름. **R7 도 여기서 함께 판정**(49번) | **← 다음 작업** |
| **3** | **알림톡** — 요청서 도착 알림. ★ 카카오 템플릿 **심사 lead time** 이 있으므로 구현·신청 일정을 별도 계획 | 미착수 |
| **4** | **가라 계정 정리 + 위너스 실계정 전 흐름 테스트** | 미착수 |

---

# ★★ Auth 운영 계약 — Supabase Confirm email = OFF 유지 ★★

> **Supabase 설정을 바꾸기 전에 반드시 이 섹션을 읽는다.**
...
| 항목 | 값 |
| --- | --- |
| **Confirm email** | **OFF** — 2026-08-11 production 대시보드 확인값 |
| 근거 | 중개사 가입에 **사업자·중개사무소 정보 제출 + 서류 업로드 + admin 수동 승인** 절차가 이미 있어, MVP 단계에서 이메일 소유 검증이 빠져도 별도 심사 장치가 존재한다 |

## 🚫 Confirm email 을 ON 으로 바꾸면 중개사 가입이 즉시 깨진다

코드로 확인한 연쇄(2026-08-11 조사):

1. `auth.signUp()` 이 **`session = null`** 을 반환한다
2. `RealtorSignUp.jsx` 의 awaiting-email 분기에서 **early return**
3. **`submitRealtorApplication()` 이 호출되지 않는다**
4. `realtor_applications` 행이 **생성되지 않는다**
5. 서류 **Storage 업로드가 실행되지 않는다**(업로드가 `submitRealtorApplication` 안에 있다)
6. 입력한 신청 정보 7개와 파일 2개가 **폼 state 에만 있다가 소실**된다
7. 이메일 인증 후 로그인해도 **pending application 이 없어** `RealtorRoute` 가
   `customer` + 신청서 없음으로 판정해 **`/` 로 리다이렉트**한다 → realtor flow 로 복귀 불가

★ **재제출 경로도 없다.** `submitRealtorApplication()` 의 호출부는 `RealtorSignUp.jsx` 한 곳뿐이고
그 앞에 `signUpWithEmail()` 이 있다. 인증을 마친 뒤 같은 폼을 다시 제출하면 이미 가입된
이메일이라 2번 분기로 되돌아온다(무한 반복).

## **→ Confirm email ON 전환 금지 — 구조 개편 선행 필요**

설정만 바꾸면 되는 일이 아니다. 아래 구조 개편이 **먼저** 끝나야 한다.

## 향후 정공법 — 가입과 중개사 지원을 분리한다

```
계정 생성 → 이메일 인증 → 로그인 → 중개사 지원서 작성 → 서류 업로드 → pending → admin 승인
```

현재는 이 전체가 **단일 폼 한 번의 제출**로 묶여 있어 세션이 없는 순간 통째로 무너진다.
분리하면 `!data?.session` 분기 자체가 사라지고, 지원서 작성은 항상 세션이 있는 상태에서만
일어난다(`submitRealtorApplication` 은 Storage 경로 생성과
`realtor_applications_insert_own` 정책 때문에 **세션이 반드시 필요**하다).

★ **고객 쪽 `PENDING_REQUEST_KEY` 패턴을 그대로 복사할 수 없다.** 사업자등록증·중개등록증
**파일 2개를 localStorage 에 담을 수 없기** 때문이다. 텍스트만 보존하고 파일은 재선택을
요구하거나 다른 방식이 필요하다.

**이 구조 개편은 Confirm email 을 실제로 ON 으로 전환할 때 함께 진행한다.**
2026-08-11 시점에는 구현하지 않았다 — pending form / localStorage / file persistence 구조도
만들지 않았다.
...
| --- | --- |
| 증상 | 중개사 가입 완료 불가 |
| 현행 blocker 여부 | **아니다** — Confirm email OFF 에서는 정상 동작 |
| **상태** | **latent / configuration-dependent** |
| 조건 | **Confirm email ON 시 즉시 P0 signup blocker** |

★ **"정상"이라고 쓰지 않는다. "OFF 라는 운영 조건 아래 정상"이다.** 설정 하나로 뒤집히는
상태이며, 그 설정은 코드가 아니라 대시보드에 있어 저장소 이력에 남지 않는다.

## 2026-08-11 에 한 일 (구현은 문구 정리뿐)

- `RealtorSignUp.jsx` awaiting-email 화면의 **거짓 안내 제거**. 이전 문구는
  *"파트너 로그인으로 다시 로그인하면 서류 제출을 이어갈 수 있어요"* 였는데
  **그런 흐름이 코드에 존재하지 않는다.**
- **분기는 유지했다.** 단순 dead code 로 보고 지우면, 설정이 실수로 ON 이 됐을 때
  사용자가 **접수된 줄 알고 기다리게 된다.** 이 화면이 "접수되지 않았다"는 사실을 알리는
  유일한 fail-safe 다.
- `handleSubmit` 의 분기 주석도 함께 고쳤다(기존 주석이 *"서류는 로그인 후 다시 제출해야 함"*
  으로 같은 거짓을 담고 있었다).

★ **이 화면은 4개 언어로 만들지 않았다.** `RealtorSignUp.jsx` 는 `useLanguage`/`translations`
import 가 **0건**이고 전 화면이 한국어 하드코딩이다(`AdminDashboard` 와 같은 상태 — 45번).
이 문구만 4개 언어로 만들면 **같은 화면에 두 체계가 공존**하게 되는데, 45번이 이미
"부분 도입은 하지 않는다"고 판단한 바로 그 상황이다. 중개사는 한국 공인중개사라
한국어 전용이 방어 가능하다.

### 파트너 화면 i18n 전환 — 별도 작업 · **우선순위 낮음**

`RealtorSignUp` / `RealtorDashboard` / `RealtorRespond` / `RealtorPending` / `AdminDashboard` /
`AdminLogin` 은 전부 한국어 하드코딩이다. 고객 화면(4개 언어)과 체계가 다르다.

- **지금 하지 않는 이유**: 대상이 국내 공인중개사·내부 운영자라 MVP 단계에서 방어 가능하다.
  영업 개시 최소 조건 4개 어디에도 들어가지 않는다.
- **하게 된다면**: 45번 원칙대로 **화면 전체를 한 번에** 전환한다. 문구 단위 부분 도입은 하지 않는다.
- **재검토 조건**: 외국인 중개사가 실제로 유입되거나, 파트너 대상 마케팅이 다국어로 나갈 때.

## 031 / 032 는 이 뒤로 미룬다

**근거**: 매물을 1개씩만 보내도 서비스는 돌아간다. 반대로 **가입이 안 되면 사용자가 0명**이라
확장 기능이 의미가 없다. 031(다중 매물 응답)과 032(요청서 제약)는 둘 다 "이미 돌아가는 흐름을
넓히는" 작업이지 "흐름을 성립시키는" 작업이 아니다.

★ **031 READY 상태는 그대로 유지된다.** P1~P8 전부 PASS 이고 백업·복원 절차가 검증돼 있으므로,
나중에 착수할 때 처음부터 다시 하지 않는다. 다만 **시간이 지나면 백업이 낡는다** — 실제 적용
시점에 `restore_rehearsal_pre031.sql` 의 **drift gate 가 자동으로 잡는다.** drift 가 잡히면
백업을 다시 뜨면 되고, **복원 절차 자체(P4)는 재검증이 필요 없다.**

## 이 변경이 함께 바꾸는 것

- **034 / R7 / T16-a 의 "2026년 11월" 시점 표기가 무의미해졌다.** 49번·50번·034 파일이
  "11월 신규 중개사 승인 플로우 검증 때"로 적고 있는데, 그 검증이 곧 위 **2번 항목**이다.
  → **시점이 11월이 아니라 "2번 작업 시점"으로 앞당겨진다.** 파일 수정은 2번 착수 시
  함께 하고, 지금은 이 항목으로 갈음한다.
- **R7(영업지역 없는 중개사 0행)** 은 2번에서 새 계정을 승인하는 순간 자연 발생하는
  상태로 판정된다(49번). 2번과 묶으면 추가 비용이 0이다.
- **T16-a(회원가입 후 profiles 행 생성)** 도 1번·2번에서 실제 가입을 하게 되므로 그때
  함께 확인한다. 다만 034 의 `handle_new_user()` revoke 는 **아직 적용하지 않은 상태**라,
  1번 가입 버그를 조사할 때 "권한 때문인가?"를 원인 후보에서 배제할 수 있다
  (030 4-2 를 제외 적용했으므로 현재 `handle_new_user` 의 EXECUTE 는 회수되지 않았다).

## 1번(가입 버그) 착수 전 미리 정리해 둔 것

다음 턴 분석의 출발점이다. **아직 조사하지 않았고 추정도 하지 않는다** — 관련 자원 위치만 적는다.

- 트리거: `on_auth_user_created` (`auth.users` AFTER INSERT) → `handle_new_user()`
  (schema.sql:57-59). migration_016 이 role 위조를 막도록 수정했다
- 현재 `handle_new_user()` EXECUTE 권한은 **회수되지 않았다**(030 4-2 제외 적용 → 034 이월)
- 가입 화면: `RealtorSignUp.jsx`, 공통 로그인 진입은 `/realtor/login`
- 신청서: `realtor_applications` (migration_013/014), 가입 전 호출되는
  `check_landline_duplicate(text)` 는 anon 접근이 의도적으로 유지돼 있다(030 4-3)
- CLAUDE.md 「로그인 페이지 profile null 처리」가 "user 는 있는데 profile 이 없는" 비정상
  상태를 다루고 있다 — Confirm email 흐름과 관련될 가능성이 있어 함께 볼 것

---

# ★★★ 2026-08-31 재개 — DB 실측 기준선 ★★★

> **이 섹션이 마이그레이션 적용 상태의 단일 진실 소스다.**
> 위쪽의 「현재 DB 상태 (2026-08-11 기준)」을 포함해 이전 상태 기록은 전부 그 시점 기록이며,
> 적용 여부를 판단할 때는 **이 섹션을 기준으로 한다.**
> 앞으로 migration 을 적용하면 이 섹션 아래에 새 실측 블록을 덧붙인다.

## 배경 — 20일 공백

마지막 커밋 `7070945` (2026-08-11 15:32) 이후 **2026-08-31 까지 20일간 작업 중단.**
재개 시점에 `git pull` = Already up to date, working tree clean.
3대 환경(집/사무실/노트북) 어디에도 미푸시 작업이 없다.

## ★ Supabase 무료 플랜 자동 일시정지 (2026-08-31 확인)

- 공백 기간 중 **무료 플랜의 비활성 프로젝트 자동 일시정지가 발생했다.**
- 재개하려면 대시보드에서 **Resume project** 를 눌러야 한다.
- ★ **이 사실이 8/11 ~ 8/31 사이 DB 무변경을 확정한다.** 일시정지된 프로젝트에는
  아무도 접속할 수 없었으므로, 아래 실측값은 8/11 마지막 작업 시점의 상태와 같다.
  실제로 seed 행 수가 8/10~8/11 기록과 **전부 일치**해 이 판단을 뒷받침한다(아래 ⑧).

### 🚨 출시 전 Pro 전환 필요

| 항목 | 내용 |
| --- | --- |
| 위험 | 무료 플랜은 일정 기간 미사용 시 **프로젝트가 자동 일시정지된다** |
| 영향 | 영업 개시 후 트래픽이 뜸한 시기에 정지되면 **서비스가 통째로 중단**된다. 고객·중개사 모두 접속 불가 |
| 조치 | **영업 개시 최소 조건 4개와 같은 등급으로 취급한다.** 실사용자를 받기 전에 Pro 전환 |
| 시점 | 아래 「영업 개시 최소 조건」 4번(위너스 실계정 전 흐름 테스트) **이전**에 완료 |

★ 개발 단계에서는 무료 플랜 유지 가능하다. 다만 **정지될 때마다 Resume 이 필요**하고,
정지 중에는 검증 세션이 통째로 막히므로 D-2 같은 장시간 검증 전에 상태를 먼저 확인한다.

## 실측 결과 (2026-08-31, SQL Editor 직접 실행)

| # | 확인 항목 | 결과 | 판정 |
| --- | --- | --- | --- |
| ① | 024/025/026/028 테이블 7개 | `districts` `lines` `stations` `station_lines` `station_districts` `station_aliases` `realtor_service_areas` **전부 존재** | 024·025·026·028 **적용** |
| ② | 027 파생 컬럼 4개 | `station_id` `district_code` `location_lat` `location_lng` **전부 존재** | 027 **적용** |
| ③ | 029 RPC 6종 | **전부 존재** | 029 **적용** |
| ④ | `requests` 정책 3개 | `requests_select_own_or_admin`(SELECT) / `requests_insert_own`(INSERT) / `requests_update_own`(UPDATE) | 030 **적용** |
| ⑤ | `properties_insert_realtor` | `with_check` 350자, `realtor_service_areas` 조인 **있음**(030 지역 체크). `count`/`5` 문자열 **없음** | 030 적용 · **031 미적용 확정** |
| ⑥ | 트리거 함수 2개 `proacl` | `handle_new_user` / `increment_response_count` 둘 다 **anon=X, authenticated=X 살아있음** | **034 미적용** (030 4-2 제외 적용과 일치) |
| ⑦ | `requests` CHECK 4개 | `deposit_range_consistency` `jeonse_loan_consistency` `location_type_station_only` `property_category_residential_only` | **032 미적용** (역 필수·extra_note 길이 제약 없음) |
| ⑧ | seed 행 수 | districts **256** / stations **308** / station_districts **318** / station_aliases **1,364** / realtor_service_areas **2** | 아래 참고 |

### ★ ④ 가 030 적용의 결정적 증거다

`requests` 의 SELECT 정책이 **`requests_select_own_or_admin`** 이다.
030 이전에는 `requests_select_own_or_realtor` 였고(2389 줄 기록), 중개사가 정책 경로로
요청서를 직접 읽을 수 있었다. 정책명에서 `realtor` 가 사라졌다는 것은
**030 의 열람 잠금이 실제로 걸려 있다**는 뜻이다 — 중개사는 이제 029 RPC 로만 접근한다.

### ★ ⑧ 이 "20일간 DB 무변경" 을 뒷받침한다

| 테이블 | 이번 실측 | 이전 기록 | 대조 |
| --- | --- | --- | --- |
| `districts` | 256 | 256 (2351·2376 줄) | **동일** |
| `stations` | 308 | 308 (2351·2376 줄) | **동일** |
| `station_districts` | 318 | 318 (2351·2376 줄) | **동일** |
| `station_aliases` | 1,364 | 1,364 (2563 줄, 2026-08-10 시드) | **동일** |
| `realtor_service_areas` | 2 | 2 (베스트=11410 / 대박=11680) | **동일** |

**drift 0.** 자동 일시정지로 접속 자체가 불가능했다는 사실과 일관된다.

## 확정 기준선

```
적용 완료 : 022 ~ 030 + 033   (030 은 4-2 두 줄 제외한 14문)
미적용    : 031 / 032 / 034
```

**2903 줄의 「현재 DB 상태 (2026-08-11 기준)」 기록이 실측과 완전히 일치한다.**
문서가 실제 DB 를 정확히 반영하고 있었다 — 기록 방식을 바꿀 이유가 없다.

★ **재개 전 기억은 "016~023, 030, 033 적용 / 024~032 미적용" 이었고, 이는 틀렸다.**
실제로는 **024~029 도 이미 적용**돼 있었다. 만약 그 기억대로 진행했다면
"지역 라우팅이 프로덕션에 없다" 는 잘못된 전제 위에서 이미 있는 테이블을 다시 만들려
했을 것이다. **공백 후 재개 시 문서 기록보다 기억을 신뢰하지 않는다** 는 것이
이번에 확인된 원칙이다.

## 이 기준선이 확정하는 것

- **031 프론트 작업 · 백업(P1~P8 READY) 상태는 그대로 유효하다.** drift 0 이므로
  `restore_rehearsal_pre031.sql` 의 drift gate 도 통과할 가능성이 높다.
  다만 **적용 시점에 반드시 다시 돌린다** — 이 실측은 drift gate 를 대체하지 않는다.
- **032 선행조건 2개 중 extra_note 300자 제한은 프론트에 이미 배포돼 있다**(`5d6d309`).
  남은 것은 LocationStep 이 `station_id` 를 실제로 저장하는지 확인뿐이다.
- **034 는 여전히 미적용이라, D-2 가입 검증에서 "권한 때문인가?" 를 원인 후보에서
  배제할 수 있다.** ⑥ 이 EXECUTE 가 살아있음을 직접 확인했다.

## 우선순위는 바뀌지 않는다

「★★ 우선순위 변경 (2026-08-11) — 영업 개시 최소 조건」 4개가 그대로 유효하다.
다음 작업은 **2번(신규 중개사 승인 플로우 검증)** 이다.
단, 위 「출시 전 Pro 전환」이 **4번 이전에 처리해야 할 항목으로 추가**된다.
